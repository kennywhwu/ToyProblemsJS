...
// Include the total cost as the last index of the array
...
function getRecipes(arr) {
  ...
  var acceptedRecipes = arr.reduce(function(acc, element) {
    if (!element.hasNuts && !element.hasMeat) {
      cost += element.cost;
      acc.push(element.recipe);
    }
    return acc;
  }, []);

  acceptedRecipes.push('Total cost is ' + cost + ' dollars.');
